...
Prompt UI Academy was conceived and built during the OpenAI Build Week submission period for the **Education** track. The project grew from PeterShanxin's original idea - teaching people the shared UI vocabulary needed to communicate precisely with AI coding agents - into a public, multi-route learning product through GPT-5.6 and Codex-assisted development.

PeterShanxin made the key product and learning-design decisions: begin with visual UI terminology, teach motion language through direct manipulation, compare vague and precise prompts, expand the first single-page experience into focused routes, add Chinese-English i18n, and publish the result as an accessible open-source project. Codex accelerated the information architecture, Next.js implementation, interactive exercises, responsive and accessible behavior, testing, CI, deployment, documentation, debugging, and dependency review.
...
1. Open the [public deployment](https://prompt-ui-academy.vercel.app) and choose English or Chinese in the header.
2. Visit `/learn` to see the three-chapter, nine-lesson learning path.
3. Use `/dictionary` to search for a component and mark one term as mastered.
4. Use `/motion` to switch animations and change duration and easing.
5. Complete a question in `/quiz` and read the immediate explanation.
6. In `/lab`, compare the vague and precise prompts, then edit the text and watch the deterministic preview and clarity score react.

No account, API key, sample data, or paid service is required for the learning experience. An optional account can sync progress between devices.
...
学习进度默认保存在浏览器本地，不需要账号；也可以选择登录，将进度安全同步到其他设备。
