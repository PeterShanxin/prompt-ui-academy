  ...
  return (
    <>
      ...
      <header className="site-header">
        <Link className="brand" href="/" aria-label={pick("界面话术首页", "Interface Language home")} onClick={() => setMenuOpen(false)}><span className="brand-mark" aria-hidden="true">介</span><span>{pick("界面话术", "Interface Language")}</span></Link>
        <nav className={menuOpen ? "menu-open" : ""} aria-label={pick("主导航", "Primary navigation")}>
          {navItems.map((item) => <Link key={item.href} className={pathname === item.href ? "active" : ""} href={item.href} onClick={() => setMenuOpen(false)}>{pick(item.zh, item.en)}</Link>)}
        </nav>
        <div className="header-actions">
          ...
          <AccountControls />
          <Link className="header-progress" href="/learn" aria-label={`${pick("当前学习进度", "Current learning progress")} ${progress}%`}><span>{pick("学习进度", "Progress")}</span><strong>{progress}%</strong><i><b style={{ width: `${progress}%` }} /></i><ArrowIcon /></Link>
      ...
      <footer><Link className="brand footer-brand" href="/"><span className="brand-mark">介</span><span>{pick("界面话术", "Interface Language")}</span></Link><p>{pick("一个帮助人类与 AI 更好协作的互动学习实验。", "An interactive experiment for better human-AI collaboration.")}</p><Link href="/learn">{pick("继续学习", "Keep learning")} →</Link></footer>
    </>
  );
}
